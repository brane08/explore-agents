...
_SYSTEM = (
    ...
    "Never use judgement words — 'properly', 'appropriately', 'gracefully', "
    "'efficiently', 'robustly', 'as expected', 'reasonable'. State the "
    "observable outcome instead: what is returned, or what is reported.\n"
    "The LAST criterion must always state failure behaviour: what the agent "
    "must do when the work cannot be done (say 'fails', 'reports the failure', "
    "or names the unavailable/invalid condition). Write it even when the task "
    "says nothing about failure. Failure means the work could not be done — a "
    "source is unreachable, an input is invalid, an operation errored. An "
    "empty result is not a failure; if the empty case matters, that is a "
    "separate criterion.\n"
    ...
    "Cover the task's main verb explicitly. If the task says summarize, "
    "compare, rank, count, or explain, one criterion must state that the "
    "output IS that — a summary, a comparison, a ranking, a count. Returning "
    "the underlying data is a different capability and does not satisfy it.\n"
    "Every criterion must trace to something the task actually asks for. Do "
    "not invent capabilities around it — no editing, writing, exporting, "
    "filtering, scheduling, or presentation features unless the task asks for "
    "them. Write the FEWEST criteria that fully cover the task; a long list is "
    "a list the work cannot be accepted against.\n"
