...
def test_security_allows_the_manifest_to_record_its_own_harness(candidate_dir):
    """§6 requires the manifest to name the harness that generated the
    candidate, and every real Role A harness id contains a model family name.
    Reading that provenance field as a hardcoded model id fails every candidate
    a real harness can produce — §3.5 forbids *pinning a model in the agent*,
    which is what model_profile exists to prevent."""
    inputs = _inputs()
    write_candidate(candidate_dir, inputs)
    manifest = candidate_dir / "AGENT_MANIFEST.yaml"
    import yaml as _yaml
    data = _yaml.safe_load(manifest.read_text(encoding="utf-8"))
    data["harness"] = "claude-code/2.1.228"
    manifest.write_text(_yaml.safe_dump(data, sort_keys=True), encoding="utf-8")
    result = step_security_static(load_candidate(candidate_dir), inputs)
    assert result.passed, result.detail


def test_security_still_catches_a_model_id_elsewhere_in_the_manifest(candidate_dir):
    """The harness exemption is one field, not the file: a model pinned
    anywhere else in the manifest is still the §3.5 violation."""
    inputs = _inputs()
    write_candidate(candidate_dir, inputs)
    manifest = candidate_dir / "AGENT_MANIFEST.yaml"
    import yaml as _yaml
    data = _yaml.safe_load(manifest.read_text(encoding="utf-8"))
    data["harness"] = "claude-code/2.1.228"
    data["routing_summary"] = "Calls claude-opus-4-8 directly."
    manifest.write_text(_yaml.safe_dump(data, sort_keys=True), encoding="utf-8")
    result = step_security_static(load_candidate(candidate_dir), inputs)
    assert not result.passed
...
def test_security_scans_the_runtime_surface_for_model_ids(candidate_dir):
    """§3.5 is about the agent pinning a model instead of resolving
    model_profile, so every file that runs or is sent to a model is scanned."""
    inputs = _inputs()
    write_candidate(candidate_dir, inputs,
                    src_body="MODEL = 'gpt-4o'\n\n\ndef run():\n    return MODEL\n")
    result = step_security_static(load_candidate(candidate_dir), inputs)
    assert not result.passed
    assert "gpt-4o" in result.detail


def test_security_scans_prompts_for_model_ids(candidate_dir):
    inputs = _inputs()
    write_candidate(candidate_dir, inputs, extra_files={
        "prompts/default.md": "You are claude-opus-4-8. Answer briefly.\n"})
    result = step_security_static(load_candidate(candidate_dir), inputs)
    assert not result.passed


@pytest.mark.parametrize("rel, body", [
    ("trace/turns.md", "turn 1: harness claude-code/2.1.228 started\n"),
    ("tests/test_no_model_ids.py",
     "def test_no_model_ids():\n"
     "    for forbidden in ('claude-', 'gpt-4', 'gemini-'):\n"
     "        assert forbidden not in open('src/component.py').read()\n"),
    ("SPEC.md", "Generated by claude-code/2.1.228.\n"),
], ids=["trace-provenance", "self-check-test", "spec-provenance"])
def test_security_does_not_read_provenance_as_a_hardcoded_model_id(
        candidate_dir, rel, body):
    """Off the runtime surface, naming a model is how a candidate *records*
    what produced it or *asserts* the rule — §6 requires the trace log, and a
    §3.5 self-check can only be written by listing the forbidden identifiers.
    Failing these fails every candidate a real harness can produce."""
    inputs = _inputs()
    write_candidate(candidate_dir, inputs, extra_files={rel: body})
    result = step_security_static(load_candidate(candidate_dir), inputs)
    assert result.passed, result.detail


def test_security_allows_the_manifest_to_record_its_own_harness(candidate_dir):
    """The manifest is on the runtime surface, but §6 requires it to name the
    harness that generated the candidate — provenance, not a model pin."""
    inputs = _inputs()
    write_candidate(candidate_dir, inputs)
    manifest = candidate_dir / "AGENT_MANIFEST.yaml"
    import yaml as _yaml
...
def test_security_still_catches_a_model_id_elsewhere_in_the_manifest(candidate_dir):
    """The harness exemption is one field, not the file."""
    inputs = _inputs()
    write_candidate(candidate_dir, inputs)
    manifest = candidate_dir / "AGENT_MANIFEST.yaml"
    import yaml as _yaml
...
def test_security_scans_every_file_for_secrets(candidate_dir):
    """Only the model-id rule is scoped. A leaked credential is a leaked
    credential wherever in the candidate it sits."""
    inputs = _inputs()
    write_candidate(candidate_dir, inputs, extra_files={
        "trace/turns.md": "used token sk-abcdefghijklmnop1234\n"})
    result = step_security_static(load_candidate(candidate_dir), inputs)
    assert not result.passed
...
# --- step 7: the flagged-tag shape the playbook tells Role A to write --------

def _set_draft_tags(root: Path, tags):
    import yaml as _yaml
    draft = root / "agents" / "_proposed" / "log-error-lister" / "entry.draft.yaml"
    data = _yaml.safe_load(draft.read_text(encoding="utf-8")) or {}
    data["capability_tags"] = tags
    draft.write_text(_yaml.safe_dump(data, sort_keys=False), encoding="utf-8")


def test_promotion_reads_the_flagged_tag_shape_without_crashing(promo_root):
    """Playbook §6 tells Role A to write new tags flagged `proposed: true`, so
    a real draft carries mappings, not bare strings. Feeding those to a set()
    of strings raises TypeError — the gate has to *refuse*, and a crash is not
    a refusal."""
    ...
    assert calls["lockbuild"] == 0, "a refused promotion must not touch the lock"
    assert (promo_root / "agents" / "_proposed" / "log-error-lister").is_dir()


def test_promotion_accepts_flagged_shape_when_every_tag_is_approved(promo_root):
    """The mapping shape itself is not the problem — an unapproved tag is."""
    _set_draft_tags(promo_root, [
        {"name": "log-source", "proposed": False},
        {"name": "log-analysis", "proposed": False},
    ])
    result, _ = _promote(promo_root)
    assert result.passed, result.detail
...
def test_promotion_still_refuses_an_unflagged_unapproved_tag(promo_root):
    """The bare-string shape stays supported, and an unknown tag in it is
    still outside the vocabulary — flagging is not what makes it unapproved."""
    _set_draft_tags(promo_root, ["log-source", "cron-scheduling"])
    result, _ = _promote(promo_root)
    assert not result.passed
    assert "cron-scheduling" in result.detail
